...
describe("unbanCommandTest", function () {
  it("Should be able to unban members to protected rooms, removing all policies that will target them", async function (this: DraupnirTestContext) {
  ...
  it("Unbans users even when there are no policies", async function (this: DraupnirTestContext) {
    const draupnir = this.draupnir;
    if (draupnir === undefined) {
      throw new TypeError(`setup didn't run properly`);
    }
  ...
  it("Unbans and reinvites users when the invite option is provided", async function (this: DraupnirTestContext) {
    const draupnir = this.draupnir;
    if (draupnir === undefined) {
      throw new TypeError(`setup didn't run properly`);
    }
  ...
  it("unbans users when there is no membership and only policies", async function (this: DraupnirTestContext) {
    const draupnir = this.draupnir;
    if (draupnir === undefined) {
      throw new TypeError(`setup didn't run properly`);
    }
  ...
  it("Unbans all members targetted by a glob that is being removed indirectly by unbanning a single user in the command", async function (this: DraupnirTestContext) {
    const draupnir = this.draupnir;
    if (draupnir === undefined) {
      throw new TypeError(`setup didn't run properly`);
    }
    ...
    const indirectlyBannedUser = await newTestUser(this.config.homeserverUrl, {
    ...
    for (const roomID of protectedRooms) {
      ...
      expect(
        membershipRevision.membershipForUser(indirectlyBannedUserID)?.membership
      ).toBe(Membership.Leave);
      expect(
        membershipRevision.membershipForUser(indirectButShouldBeBannedUserID)
          ?.membership
  ...
  it("removes glob rules when there are only glob rules and no banned members", async function (this: DraupnirTestContext) {
    const draupnir = this.draupnir;
    if (draupnir === undefined) {
      throw new TypeError(`setup didn't run properly`);
    }
