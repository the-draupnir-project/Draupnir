...
import { ActionException, ActionExceptionKind, ActionResult, EventDecoder, Logger, Ok, RoomStateBackingStore, RoomStateRevision, StateChange, StateEvent, StringRoomID, isError } from "matrix-protection-suite";
...
export class SqliteRoomStateBackingStore extends BetterSqliteStore implements RoomStateBackingStore {
    private readonly roomInfoMap = new Map<StringRoomID, RoomInfo>;
    public readonly revisionListener = this.handleRevision.bind(this);
    public constructor(path: string, private readonly eventDecoder: EventDecoder) {
        super(schema.map(text => function(db) {
            db.prepare(text).run();
        }), {
            path,
            fileMustExist: false,
    ...
    public handleRevision(revision: RoomStateRevision, changes: StateChange[]): void {
        ...
        const doCompleteWriteback = this.db.transaction(() => {
            const info: RoomInfo = { room_id: revision.room.toRoomIDOrAlias(), last_complete_writeback: Date.now() };
    ...
    private getRoomMeta(roomID: StringRoomID): RoomInfo | undefined {
        ...
        if (entry) {
            ...
            const dbEntry = this.db.prepare(`SELECT * FROM room_info WHERE room_id = ?`).get(roomID) as RoomInfo | undefined;
    ...
    public getRoomState(
        ...
        if (roomInfo === undefined) {
            ...
            const events = [];
            for (const event of this.db.prepare(`SELECT json(event) FROM room_state_event WHERE room_id = ?`).pluck().iterate(roomID) as IterableIterator<string>) {
