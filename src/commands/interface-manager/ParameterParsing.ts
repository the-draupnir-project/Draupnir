...
export interface IArgumentStream extends SuperCoolStream<ReadItem, ReadItem[]> {
  ...
  ): Promise<ActionResult<ReadItem>>;
}

export class ArgumentStream
  extends StandardSuperCoolStream<ReadItem, ReadItem[]>
  ...
  return (readItem: ReadItem) => {
    ...
    if (result) {
      ...
      return ActionError.Result(
        `Was expecting a match for the presentation type: ${name} but got ${printReadably(readItem)}.`
      );
    }
  };
}
...
export function presentationTypeOf(
  ...
  const candidates = [...PRESENTATION_TYPES.values()].filter(
    (possibleType) =>
      possibleType.validator(presentation as ReadItem).isOkay &&
  ...
  if (candidates.length === 0) {
    ...
    throw new TypeError(
      ...
      `presentationTypeof: There are multiple candidates for the presentation ${presentation}: ${JSON.stringify(candidates.map((c) => c.name))}`
    );
  }
}
    ...
    while (stream.peekItem(undefined) !== undefined) {
      ...
      if (keywordValue !== undefined) {
        ...
        if (isError(validationResult)) {
          return ArgumentParseError.Result(validationResult.error.message, {
            parameter: this,
            stream,
          });
        }
...
class KeywordParser {
  ...
  ): ActionResult<ReadItem | true, ArgumentParseError> {
    if (
  ...
  ): Promise<ActionResult<ReadItem[] | undefined>> {
...
export type ParameterParser = (
  stream: IArgumentStream
) => Promise<ActionResult<ParsedArguments>>;
...
class ArgumentListParser implements IArgumentListParser {
  ...
  public async parse(
    ...
    for (const parameter of this.descriptions) {
      ...
      if (isError(result)) {
        return ArgumentParseError.Result(result.error.message, {
          parameter,
          stream,
        });
      }
...
export class AbstractArgumentParseError extends ActionError {
  ...
  ): ActionResult<never, AbstractArgumentParseError> {
...
export class ArgumentParseError extends AbstractArgumentParseError {
  ...
  ): ActionResult<Ok, ArgumentParseError> {
    return ResultError(
...
export class UnexpectedArgumentError extends AbstractArgumentParseError {
  public static Result<Ok>(
    message: string,
    options: { stream: IArgumentStream }
  ): ActionResult<Ok, UnexpectedArgumentError> {
