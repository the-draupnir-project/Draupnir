...
> implements InterfaceAcceptor
{
  ...
  public async invoke(
    ...
    > = await this.interfaceCommand.parseThenInvoke(executorContext, stream);
    ...
    if (isError(executorResult)) {
      if (executorResult.error instanceof PromptRequiredError) {
        const parameter = executorResult.error
          .parameterRequiringPrompt as ParameterDescription<ExecutorType>;
        ...
        if (promptOptions.default) {
          await promptDefault.call(
            matrixContext,
            parameter,
            this.interfaceCommand,
            promptOptions.default,
            args
          );
        } else {
          await promptSuggestions.call(
            matrixContext,
            parameter,
            this.interfaceCommand,
  ...
  private async reportValidationError(
    ...
    LogService.info(
      ...
      `User input validation error when parsing command ${JSON.stringify(this.interfaceCommand.designator)}: ${validationError.message}`
    );
...
const MATRIX_INTERFACE_ADAPTORS = new Map<
  InterfaceCommand,
  MatrixInterfaceAdaptor<MatrixContext>
>();

function internMatrixInterfaceAdaptor(
  interfaceCommand: InterfaceCommand,
  adapator: MatrixInterfaceAdaptor<MatrixContext>
): void {
  if (MATRIX_INTERFACE_ADAPTORS.has(interfaceCommand)) {
    throw new TypeError(
      `An adaptor is already defined for the command ${interfaceCommand.designator.toString()}`
    );
  }
...
export function findMatrixInterfaceAdaptor(
  ...
  throw new TypeError(
    `Couldn't find an adaptor for the command ${interfaceCommand.designator.toString()}`
  );
}
