  ...
  public importTable(table: CommandTable): void {
    for (const command of table.getAllCommands()) {
      if (this.findAMatchingCommand(new ArgumentStream(command.designator))) {
        throw new TypeError(
          `Command ${JSON.stringify(command.designator)} is in conflict with this table and cannot be imported.`
        );
      }
    }
...
export function findTableCommand<ExecutorType extends BaseFunction>(
  ...
  if (
    ...
    throw new TypeError(
      `Could not find a table command in the table ${tableName.toString()} with the designator ${JSON.stringify(designator)}`
    );
  }
