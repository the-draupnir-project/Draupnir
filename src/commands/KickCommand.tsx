...
import { ActionError, ActionResult, MatrixRoomReference, Ok, StringRoomID, StringUserID, UserID, isError } from "matrix-protection-suite";
...
function addUserToKick(map: UsersToKick, roomID: StringRoomID, userID: StringUserID): UsersToKick {
...
export async function kickCommand(
    ...
    const isDryRun = this.draupnir.config.noop || (keywords.getKeyword<string>("dry-run", "false") === "true");
    const allowGlob = keywords.getKeyword<string>("glob", "false");
    const isGlob = user.toString().includes('*')  || user.toString().includes('?');
    if (isGlob && !allowGlob) {
        return ActionError.Result("Wildcard bans require an additional argument `--glob` to confirm");
    }
