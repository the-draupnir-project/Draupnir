...
import {
  ...
  findTableCommand,
} from "../../src/commands/interface-manager/InterfaceCommand";
import {
  ArgumentStream,
  findPresentationType,
  parameters,
  union,
} from "../../src/commands/interface-manager/ParameterParsing";
...
it("A command that parses mxids", async function () {
  ...
  const result = await command.parseThenInvoke(
    undefined,
    new ArgumentStream(readCommand("@spam:example.com"))
  );
  if (isError(result)) {
