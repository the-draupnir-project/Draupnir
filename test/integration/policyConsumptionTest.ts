...
/**
 *  We also need a test for when the server sends "reset" and that
 *  when the rules appear in both add and stop, nothing happens or
 *  stop happens before add.
 * 
 *  FIXME: Banning an alias doesn't resolve the alias before creating the RuleServer rule
 */
describe("Test: that policy lists are consumed by the associated synapse module", function () {
    ...
    it('blocks users in antispam when they are banned from sending messages and invites serverwide.', async function() {
        ...
        const banList = await createBanList(config.managementRoom, mjolnir, moderator);
        await badUser.sendMessage(unprotectedRoom, {msgtype: 'm.text', body: 'Something bad and mean'});

        await waitForRuleChange(async () => {
            await getFirstReaction(mjolnir, config.managementRoom, '✅', async () => {
                return await moderator.sendMessage(config.managementRoom, { msgtype: 'm.text', body: `!mjolnir ban ${banList} ${badUserId}` });
            });
        });
        ...
        await waitForRuleChange(async () => {
            await getFirstReaction(mjolnir, config.managementRoom, '✅', async () => {
                return await moderator.sendMessage(config.managementRoom, { msgtype: 'm.text', body: `!mjolnir unban ${banList} ${badUserId}` });
            });
        });
    ...
    it('Test: Cannot send message to a room that is listed in a policy list and cannot invite a user to the room either', async function () {
        ...
        await waitForRuleChange(async () => {
            await getFirstReaction(mjolnir, config.managementRoom, '✅', async () => {
                return await moderator.sendMessage(config.managementRoom, { msgtype: 'm.text', body: `!mjolnir ban ${banList} ${badRoom}` });
            });
        });
        ...
        await waitForRuleChange(async () => {
            await getFirstReaction(mjolnir, config.managementRoom, '✅', async () => {
                return await moderator.sendMessage(config.managementRoom, { msgtype: 'm.text', body: `!mjolnir unban ${banList} ${badRoom}` });
            });
        });

        assert.ok(await badUser.sendMessage(badRoom, { msgtype: 'm.text', body: 'test'}), 'should not be able to send messagea to a room which is listed.');
        assert.ok(await badUser.inviteUser(await moderator.getUserId(), badRoom), 'should not be able to invite people to a listed room.');
    })
});
